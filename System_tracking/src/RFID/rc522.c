...
/*  Transacción de escritura, address,data0,data1...
*   Transacción de lectura, tx address0,address1,...
                            rx    x    ,data0   ,data1...
    en address bit 7 = 1 lectura; =0 escritura ; address de bit 6-1
*/
...
esp_err_t rc522_spi_init(int miso_io, int mosi_io, int sck_io, int cs) {
...
esp_err_t rc522_write_n(uint8_t addr, uint8_t n, uint8_t *data) {
    ...
    buffer[0] = (addr << 1) & 0x7E; //me aseguro que sea write
...
esp_err_t rc522_init() {
    ...
    res = rc522_fw_version();

    if(res != 0x92)
    {
        ESP_LOGE("RFID","RFID NO DETECTADO");
        return ESP_ERR_NOT_FOUND;
    }

    ESP_LOGD(TAG, "RC522 Firmware 0x%x\n", res);
    return ESP_OK;
}
...
esp_err_t rc522_resume() {
    return rc522_timer_running ? ESP_OK : esp_timer_start_periodic(rc522_timer, 100000);
}
